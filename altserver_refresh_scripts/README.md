...
Requires: PowerShell 7, Python 3.12 + `pymobiledevice3`, USB-trusted unlocked iPhone. `Get-IphoneLanIpv4.py` turns on Wi-Fi lockdown over USB when it is off (`lockdown wifi-connections --state on`). If Bonjour is still empty, it tries turning the Wi-Fi radio on over USB (`SetWiFiPowerState`); a denial is a short one-line reason (often unsupervised/MDM), not a traceback. It does not join an SSID for you.
